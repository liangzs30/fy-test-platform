...
<script>
import crudkwCategory from '@/api/testPlatform/kwCategory'
...
export default {
  ...
  cruds() {
    return CRUD({ title: '关键字模块', url: '/api/kwCategory', crudMethod: { ...crudkwCategory }})
  },
  ...
  data() {
    return {
      ...
      permission: {
        add: ['admin', 'kwCategory:add'],
        edit: ['admin', 'kwCategory:edit'],
        del: ['admin', 'kwCategory:del']
      },
      enabledTypeOptions: [
  ...
  methods: {
    getCategoryDatas(tree, treeNode, resolve) {
      ...
      setTimeout(() => {
        crudkwCategory.getCategorys(params).then(res => {
          resolve(res.content)
        })
      }, 100)
    },
    ...
    getSupcategorys(id) {
      crudkwCategory.getCategorySuperior(id, true).then(res => {
    ...
    getCategorys() {
      crudkwCategory.getCategorys({}).then(res => {
    ...
    loadCategorys({ action, parentNode, callback }) {
      if (action === LOAD_CHILDREN_OPTIONS) {
        crudkwCategory.getCategorys({ pid: parentNode.id }).then(res => {
