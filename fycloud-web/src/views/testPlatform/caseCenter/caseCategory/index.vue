<template>
  <div class="app-container">
    ...
    <!-- <el-button @click="printID">点击</el-button> -->
    <!--表单组件-->
...
import crudCaseCategory from '@/api/testPlatform/caseCategory'
...
const defaultForm = { id: null, name: null, isTop: '1', subCount: 0, pid: null, castegorySort: 999, enabled: 'true' }
export default {
  name: 'CaseCategory',
  components: { Treeselect, crudOperation, rrOperation, udOperation, DateRangePicker },
  computed: {
    ...mapGetters(['userProjectID'])
  },
  cruds() {
    return CRUD({ title: '用例模块', url: '/api/caseCategory', crudMethod: { ...crudCaseCategory }})
  },
  ...
  data() {
    return {
      ...
      permission: {
        add: ['admin', 'caseCategory:add'],
        edit: ['admin', 'caseCategory:edit'],
        del: ['admin', 'caseCategory:del']
      },
      enabledTypeOptions: [
  ...
  methods: {
    getCategoryDatas(tree, treeNode, resolve) {
      ...
      setTimeout(() => {
        crudCaseCategory.getCategorys(params).then(res => {
          resolve(res.content)
        })
      }, 100)
    },
    ...
    getSupcategorys(id) {
      crudCaseCategory.getCategorySuperior(id, true).then(res => {
    ...
    getCategorys() {
      crudCaseCategory.getCategorys({ projectID: sessionStorage.getItem('userProjectID') }).then(res => {
    ...
    loadCategorys({ action, parentNode, callback }) {
      if (action === LOAD_CHILDREN_OPTIONS) {
        crudCaseCategory.getCategorys({ pid: parentNode.id }).then(res => {
    ...
    printID(){
      console.log('pid: ', this.crud.params)
    }
  }
}
</script>
