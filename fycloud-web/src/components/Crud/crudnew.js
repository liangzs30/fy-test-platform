...
function CRUD(options) {
  const defaultOptions = {
    ...
    query: {},
    // 查询数据的参数
    params: {projectID: sessionStorage.getItem("userProjectID")},
    // Form 表单
    form: {},
    // 重置表单
    defaultForm: () => {},
  ...
  const data = {
    ...
    status: {
      ...
      copy: CRUD.STATUS.NORMAL,
      // 添加或编辑状态
      get cu() {
        if (this.add === CRUD.STATUS.NORMAL && this.edit === CRUD.STATUS.NORMAL && this.copy === CRUD.STATUS.NORMAL) {
          return CRUD.STATUS.NORMAL
        } else if (this.add === CRUD.STATUS.PREPARED || this.edit === CRUD.STATUS.PREPARED || this.copy === CRUD.STATUS.PREPARED) {
          return CRUD.STATUS.PREPARED
        } else if (this.add === CRUD.STATUS.PROCESSING || this.edit === CRUD.STATUS.PROCESSING || this.copy === CRUD.STATUS.PROCESSING) {
      ...
      // 标题
      get title() {
        return this.add > CRUD.STATUS.NORMAL ? `新增${crud.title}` : this.edit > CRUD.STATUS.NORMAL ? `编辑${crud.title}` : this.copy > CRUD.STATUS.NORMAL ? `复制${crud.title}` : crud.title
      }
    },
    msg: {
      submit: '提交成功',
  ...
  const methods = {
    ...
    copySuccessNotify() {
      crud.notify(crud.msg.copy, CRUD.NOTIFICATION_TYPE.SUCCESS)
    },
    delSuccessNotify() {
    ...
    toCopy(data) {
      crud.resetForm(JSON.parse(JSON.stringify(data)))
      if (!(callVmHook(crud, CRUD.HOOK.beforeToCopy, crud.form) && callVmHook(crud, CRUD.HOOK.beforeToCU, crud.form))) {
        return
      }
      crud.status.copy = CRUD.STATUS.PREPARED
      callVmHook(crud, CRUD.HOOK.afterToCopy, crud.form)
      callVmHook(crud, CRUD.HOOK.afterToCU, crud.form)
    },
    /**
   * 查看详情
   ...
   */
    // toDetail(data) {
    //   console.log('crud.status', crud.status)
    //   crud.resetForm(JSON.parse(JSON.stringify(data)))
    //   if (!(callVmHook(crud, CRUD.HOOK.beforeToDetail, crud.form) && callVmHook(crud, CRUD.HOOK.beforeToCU, crud.form))) {
    //     return
    //   }
    //   crud.status.edit = CRUD.STATUS.PREPARED
    //   crud.getDataStatus(crud.getDataId(data)).detail = CRUD.STATUS.PREPARED
    ...
    cancelCU() {
      ...
      const copyStatus = crud.status.copy
      ...
      if (copyStatus === CRUD.STATUS.PREPARED) {
        if (!callVmHook(crud, CRUD.HOOK.beforeEditCancel, crud.form)) {
          return
        }
        crud.status.copy = CRUD.STATUS.NORMAL
        crud.getDataStatus(crud.getDataId(crud.form)).copy = CRUD.STATUS.NORMAL
      }
      crud.resetForm()
      if (addStatus === CRUD.STATUS.PREPARED) {
      ...
      if (copyStatus === CRUD.STATUS.PREPARED) {
        callVmHook(crud, CRUD.HOOK.afterCopyCancel, crud.form)
      }
      // 清除表单验证
    ...
    submitCU() {
      ...
      crud.findVM('form').$refs['form'].validate(valid => {
        ...
        if (crud.status.add === CRUD.STATUS.PREPARED) {
          ...
          crud.doEdit()
        }else if (crud.status.copy === CRUD.STATUS.PREPARED) {
          crud.doCopy()
        }
      })
    },
    ...
    doCopy() {
      ...
      crud.status.copy = CRUD.STATUS.PROCESSING
      crud.crudMethod.copy(crud.form).then(() => {
        crud.status.copy = CRUD.STATUS.NORMAL
        crud.resetForm()
        crud.copySuccessNotify()
        callVmHook(crud, CRUD.HOOK.afterSubmit)
        crud.toQuery()
      }).catch(() => {
        crud.status.copy = CRUD.STATUS.PREPARED
...
CRUD.HOOK = {
  ...
  /** 编辑取消 - 之后 */
  afterCopyCancel: 'afterCrudCopyCancel',
  /** 提交 - 之前 */
